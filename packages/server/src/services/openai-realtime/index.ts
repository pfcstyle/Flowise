...
const buildAndInitTool = async (chatflowid: string, user: any, token?: string, _chatId?: string, _apiMessageId?: string) => {
...
const getAgentTools = async (chatflowid: string, user: any, token?: string): Promise<any> => {
    try {
        const agent = await buildAndInitTool(chatflowid, user, token)
...
const executeAgentTool = async (
    ...
    try {
        const agent = await buildAndInitTool(chatflowid, user, token, chatId, apiMessageId)
        const tools = agent.tools
